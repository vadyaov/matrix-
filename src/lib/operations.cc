...
static double Multiply(const S21Matrix& left, const S21Matrix& right,
                       const int i, const int j) noexcept {
  double ij{0.0};
  for (int k = 0; k < left.get_cols(); ++k) ij += left(i, k) * right(k, j);
  return ij;
}

static S21Matrix SimpleMinor(const S21Matrix& other) noexcept {
  S21Matrix tmp{2, 2};
  tmp(0, 0) = other(1, 1);
  tmp(0, 1) = -other(1, 0);
  tmp(1, 0) = -other(0, 1);
...
static S21Matrix CreateSmaller(const S21Matrix& other, const int i,
                               const int j) noexcept {
  S21Matrix smaller{other.get_rows() - 1, other.get_cols() - 1};
  const int sz = other.get_rows();
...
static double CalcMinor(const S21Matrix& other, const int i,
                        const int j) noexcept {
...
bool S21Matrix::EqMatrix(const S21Matrix& other) const noexcept {
  ...
  if (true == sz) {
    ...
    while (m1 != end && sz) sz = std::fabs(*m1++ - *m2++) < eps;
  }
  return sz;
}
...
void S21Matrix::SubMatrix(const S21Matrix& other) { SumMatrix(-other); }

void S21Matrix::MulNumber(const double n) noexcept {
  for (int i = 0; i < size(); ++i) matrix_[i] *= n;
}
...
S21Matrix S21Matrix::Transpose() const {
  ...
  for (int i = 0; i < res.rows_; ++i)
    for (int j = 0; j < res.cols_; ++j) res(i, j) = operator()(j, i);
  return res;
}
...
S21Matrix S21Matrix::CalcComplements() const {
  if (rows_ != cols_)
    throw std::runtime_error(
        "not square matrix in S21Matrix::CalcComplements()");
  if (rows_ == 1)
    throw std::runtime_error(
...
S21Matrix S21Matrix::InverseMatrix() const {
  ...
  if (std::fabs(det) < eps)
    throw std::runtime_error(
        "Determinant() is 0 in S21Matrix::InverseMatrix()");
