...
TEST(DefaultConstructorTest, Default0) {
  ...
  EXPECT_EQ(DefaultMatrix.get_rows(), 0);
  EXPECT_EQ(DefaultMatrix.get_cols(), 0);
}
...
TEST(ParamConstructorTest, NormalParams1) {
  ...
  for (auto i = 0; i < rows; ++i)
    for (auto j = 0; j < cols; ++j) EXPECT_EQ(m(i, j), 0.0);
}
...
TEST(ParamConstructorTest, Exception1) {
  try {
    ...
    EXPECT_EQ(e.what(),
              std::string("bad size in S21Matrix::S21Matrix(int, int)"));
  }
}
...
TEST(CopyConstructor, Test1) {
  S21Matrix a;
  const auto a_rows = a.get_rows();
  const auto a_cols = a.get_cols();
  const auto a_sz = a.size();

  RandMatrix(a);

  S21Matrix b{a};
  const auto b_rows = b.get_rows();
  const auto b_cols = b.get_cols();
  ...
  for (auto i = 0; i < a_rows; ++i)
    for (auto j = 0; j < a_cols; ++j) EXPECT_EQ(a(i, j), b(i, j));
}
...
TEST(MoveConstructorOperator, Test1) {
  ...
  const auto rows = a.get_rows();
  const auto cols = a.get_cols();
...
TEST(SumMatrix, Test1) {
  ...
  for (auto i = 0; i != c.get_rows(); ++i)
    for (auto j = 0; j != c.get_cols(); ++j)
...
TEST(MulNumber, Test1) {
  ...
  const auto row = a.get_rows();
  const auto col = a.get_cols();
  ...
  for (auto i = 0; i < row; ++i)
    for (auto j = 0; j < col; ++j) EXPECT_EQ(a(i, j), c(i, j) * num);
}
...
TEST(MulMatrix, Test1) {
  ...
  for (auto i = 0; i < a.get_rows(); ++i)
    for (auto j = 0; j < a.get_cols(); ++j)
...
TEST(MulNumber, Exception1) {
  ...
  try {
    ...
    EXPECT_EQ(e.what(),
              std::string("Incorrect matrix dimensions for multiplication"));
  }
}
...
TEST(Transpose, Test1) {
  ...
  EXPECT_EQ(transp.get_rows(), exp.get_rows());
  EXPECT_EQ(transp.get_cols(), exp.get_cols());

  for (auto i = 0; i < exp.get_rows(); ++i)
    for (auto j = 0; j < exp.get_cols(); ++j)
...
TEST(CalcComplements, Test1) {
  ...
  EXPECT_EQ(res.get_rows(), exp.get_rows());
  EXPECT_EQ(res.get_cols(), exp.get_cols());

  for (auto i = 0; i < exp.get_rows(); ++i)
    for (auto j = 0; j < exp.get_cols(); ++j) EXPECT_EQ(res(i, j), exp(i, j));
}
...
TEST(CalcComplements, Exception1) {
  ...
  try {
    ...
    EXPECT_EQ(e.what(),
              std::string("not square matrix in S21Matrix::CalcComplements()"));
  }
}
...
TEST(CalcComplements, Exception2) {
  ...
  try {
    ...
    EXPECT_EQ(
        e.what(),
        std::string(
            "no complements for 1x1 matrix in S21Matrix::CalcComplements()"));
  }
}
...
TEST(Determinant, Exception1) {
  ...
  try {
    ...
    EXPECT_EQ(e.what(),
              std::string("rows_ != cols_ in S21Matrix::Determinant()"));
  }
}
...
TEST(InverseMatrix, Exception) {
  ...
  try {
    ...
    EXPECT_EQ(e.what(),
              std::string("Determinant() is 0 in S21Matrix::InverseMatrix()"));
  }
}
...
TEST(OperatorPlus, Test) {
  ...
  EXPECT_EQ(c.get_rows(), 3);
  EXPECT_EQ(c.get_cols(), 3);
...
TEST(OperMulEqMat, Test) {
  ...
  for (int i = 0; i < a.get_rows(); ++i)
    for (int j = 0; j < a.get_cols(); ++j) EXPECT_EQ(a(i, j), c(i, j));
}
...
TEST(UnaryMinus, Test) {
  ...
  for (int i = 0; i < a.get_rows(); ++i)
    for (int j = 0; j < a.get_cols(); ++j) EXPECT_EQ(a(i, j), -b(i, j));
}
